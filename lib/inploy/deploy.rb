module Inploy
  class Deploy
    ...
    def remote_setup
      remote_run "cd #{path} && #{sudo_if_should}git clone --depth 1 #{repository} #{application} && cd #{application_folder} #{checkout}#{bundle} && #{sudo_if_should}bundle exec rake inploy:local:setup RAILS_ENV=#{environment} environment=#{environment}#{skip_steps_cmd}"
    end
    ...
    def remote_update
      remote_run "cd #{application_path} && #{sudo_if_should}bundle exec rake inploy:local:update RAILS_ENV=#{environment} environment=#{environment}#{skip_steps_cmd}"
    end

    def remote_rake(task)
      remote_run "cd #{application_path} && bundle exec rake #{task} RAILS_ENV=#{environment}"
    end
