  ...
  module Templates
    module Rails3Push
      ...
      def remote_setup
        ...
        command = []
        command << "cd #{application_path}"
        command << "bundle exec rake inploy:local:setup RAILS_ENV=#{environment} environment=#{environment}#{skip_steps_cmd}"
        remote_run command.join(' && ')
      end
      ...
      def remote_update
        ...
        command = []
        command << "cd #{application_path}"
        command << "bundle exec rake inploy:local:update RAILS_ENV=#{environment} environment=#{environment}#{skip_steps_cmd}"
        remote_run command.join(' && ')
      end
